...
class DuelingDDQNAgent(Agent):
    ...
    def __init__(self, input_shape, n_actions, optimizer='RMSprop', lr=1e-4, gamma=0.99, C=1000,
                 batch_size=32, min_eps=0.01, max_eps=1, cutoff=1e6, device='GPU', checkpoint_path='scores'):
        super(DuelingDDQNAgent, self).__init__(input_shape, n_actions, gamma, batch_size,
                                               min_eps, max_eps, cutoff, device, checkpoint_path)
    ...
    def choose_action(self, state):
        ...
        # state = np.array([state], copy=False, dtype=np.float32)
        # ten = torch.from_numpy(state).float().to(self.device)
        # return torch.argmax(self.policy_network.forward(ten)).item()
    ...
    def calculate_loss_and_backprop(self):
        ...
        policy_out = self.policy_network(states)[indices, actions]
        ...
        y = rewards + self.gamma * target_out[indices, max_actions]
        self.optimizer.zero_grad()
        loss = self.criterion(y, policy_out)
        loss.backward()
        self.optimizer.step()

        # after it has finished just update all neccessary values
